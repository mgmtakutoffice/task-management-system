...
from overdue_reminder_service import run_overdue_reminders  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Send one consolidated daily overdue-task reminder per active user."
        )
    )
    parser.add_argument(
        "--dry-run",
...
def main() -> int:
    ...
    try:
        ...
        print(f"ERROR: unexpected overdue reminder failure: {exc}", file=sys.stderr)
        return 1

    mode = "DRY RUN" if args.dry_run else "SEND"
    print(
        f"[{mode}] office date={result.today.isoformat()} "
        f"overdue tasks={result.eligible_tasks} "
    ...
    if result.failed_recipients:
        print(
            "ERROR: overdue reminder delivery failed for: "
