...
import overdue_reminder_service as overdue
...
def test_collect_overdue_routes_checking_to_checker_and_excludes_non_overdue_states(tmp_path):
    ...
    repo.add_task(task("TODAY", **{"Due Date": "2026-08-17"}))
    repo.add_task(task("FUTURE", **{"Due Date": "2026-08-18"}))
    ...
    grouped, warnings = overdue.collect_overdue_tasks(
        repo,
    ...
    assert grouped["associate@example.com"][0]["_days_overdue"] == "2"
...
def test_duplicate_protection_is_per_calendar_day(tmp_path):
    repo = make_repo(tmp_path)
    repo.add_task(task("A1", **{"Overdue Reminder Sent On": "2026-08-17"}))
    repo.add_task(task("A2", **{"Overdue Reminder Sent On": "2026-08-16"}))

    grouped, _ = overdue.collect_overdue_tasks(
        repo,
        today=date(2026, 8, 17),
    )
...
def test_run_sends_one_digest_per_user_marks_tasks_and_logs_activity(tmp_path, monkeypatch):
    repo = make_repo(tmp_path)
    repo.add_task(task("A1"))
    repo.add_task(task("A2", **{"Priority": "Urgent", "Due Date": "2026-08-10"}))
    repo.add_task(
        task(
            "C1",
            Status="Pending for Checking",
    ...
    monkeypatch.setattr(overdue, "_send_with_gmail_api", fake_gmail_send)
    config = {
    ...
    associate_message = next(m for m in sent_messages if m["To"] == "associate@example.com")
    assert "2 Overdue Tasks" in associate_message["Subject"]

    saved = {row["Task ID"]: row for row in repo.get_tasks()}
    assert saved["A1"]["Overdue Reminder Sent On"] == "2026-08-17"
    assert saved["A2"]["Overdue Reminder Sent On"] == "2026-08-17"
    assert saved["C1"]["Overdue Reminder Sent On"] == "2026-08-17"
    assert saved["A1"]["Overdue Reminder Sent At"]
    ...
    assert all("Daily overdue digest" in row["Additional Information"] for row in activity_rows)


def test_daily_repeat_allows_next_day_email(tmp_path):
    repo = make_repo(tmp_path)
    repo.add_task(task("A1", **{"Overdue Reminder Sent On": "2026-08-17"}))

    grouped_same_day, _ = overdue.collect_overdue_tasks(repo, today=date(2026, 8, 17))
    grouped_next_day, _ = overdue.collect_overdue_tasks(repo, today=date(2026, 8, 18))

    assert grouped_same_day == {}
    assert [row["Task ID"] for row in grouped_next_day["associate@example.com"]] == ["A1"]
...
def test_dry_run_does_not_send_or_update(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(overdue, "_send_with_gmail_api", should_not_send)
    result = overdue.run_overdue_reminders(
        repo,
        {"GMAIL_FROM_EMAIL": "reminders@example.com"},
        today=date(2026, 8, 17),
    ...
    assert repo.get_tasks()[0]["Overdue Reminder Sent On"] == ""
